...
def percentile(a, weights=None, perc=np.array([16.,84.])):
    ...
    output: array like
        ...
        If the percentile falls between two values, linear interpolation
        between the nearest values will be performed
    """
    ...
    # create the output array: output[:,1:] correspond to the columns of 'a',
    # the rows are of the size of 'perc' ordered from the lower to the upper
    # score
    output = np.zeros((perc.shape[0],a.shape[1]+1))  
...
def GR_criterion(chains):
    ...
    ----------
    chains: list
        list of arrays, each one containing a single MCMC chain with the
        structure: weight, likelihood, parameters
    ...
    # it will contain the various in-chain mean and variances^2 for the
    # parameters
    ch_mean, ch_var = np.empty((len(chains), chains[0].shape[1]-2)),\
            np.empty((len(chains), chains[0].shape[1]-2))  
