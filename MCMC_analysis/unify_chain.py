...
def options(p):
  ...
  p.add_option("-f", "--fraction", action="store", dest="fraction", type="float", default="0.5", help="The first 'FRACTION' of the chain that is going to be discarted. 'FRACTION' must be [0,1]. [default: %default]")
  p.add_option("-m", "--marginalized", action="store_true", dest="marg", help="If given, the program reads the file containing the parameter names and writes a file with the 1D marginalized constraints.")
  p.add_option("-p", "--percentile", action="append", type="float", dest="percentile", help="It allow to set the percentile for the output 1D marginalized contraints. If called more then once allows to give more than one percentile value. 'PERCENTILE' must be [0,100]. If PERCENTILE=68, gives back the lower and upper 1-sigma constraints. [default: 68.0]. To provide more the one percentile, call the option more than once")
  p.add_option("--extension", action="store", dest="ext", type="string", default="txt", help="Extension of the input and output files. [default: %default]")
  p.add_option("-d", "--discard", action="store", dest="discard", type="string", help="Discard files containing a certain string")
  p.add_option("-o", "--output-dir", action="store", dest="outdir", type="string", help="If an output directory given, the input path is substitutes with the output one.")
  p.add_option("-c", "--convergence", action="store_true", dest="conv", default=False, help="If selected, the Gelman & Rubin convergency test run. The value of R for every parameters will be printed in the file 'file_root.conv'")
...
if __name__ == "__main__":   # if is the main
  ...
  np.savetxt(outfile, tot_chain, delimiter='\t')   #output chain saved
